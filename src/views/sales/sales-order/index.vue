    ...
    <el-row :gutter="24">
      ...
      >
        ...
        <el-table-column
          ...
          prop="customerName"
        />
        <el-table-column label="销售数量" sortable align="center" prop="qty" />
...
import {
  ...
  exportSalesOrder,
  changeStatus,
} from "@/api/sales/sales-order/salesOrder.js";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        qty: [{ required: true, message: "销售数量不能为空", trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSalesOrder(this.addDateRange(this.queryParams, this.dateRange)).then(
        (response) => {
          this.salesOrderList = response.result.items;
